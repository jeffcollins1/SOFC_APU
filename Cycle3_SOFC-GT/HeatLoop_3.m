function [HeatLoop,F1,F2,F3,F4,E1,E2,E3,E4,A4] = HeatLoop_3(intake,options,FCArray,A3)
...
HeatLoop.Qairin1 = intake.heat_added;
Qtotal = HeatLoop.Qairin1 + HeatLoop.Qinfuel2;
...
for i = 1:10
    for j = 1:10
if A4.O2(i,j) > 2*E1.H2(i,j)
    dH(i,j) = FCArray.hrxnmol(1,1)*E1.H2(i,j);
    E2.H2(i,j) = 0;
    E2.H2O(i,j) = ET.H2O(i,j) + ET.H2(i,j);
    E2.O2(i,j) = A4.O2(i,j) - 2*E1.H2(i,j);
    E2.N2(i,j) = A4.N2(i,j); 
else 
    dH(i,j) = 2*FCArray.hrxnmol(1,1)*A4.O2(i,j);
    E2.O2(i,j) = 0;
    E2.H2O(i,j) = ET.H2O(i,j) + 2*ET.O2(i,j);
    E2.H2(i,j) = E1.H2(i,j) - 0.5*A4.O2(i,j); 
    E2.N2(i,j) = A4.N2(i,j); 
end
    end
end
 
H = property(E1,'h','kJ') + property(A4,'h','kJ') + dH; 
...
HeatLoop.Qexcess = FCArray.Qgen - HeatLoop.Qoutanode - HeatLoop.Qoutcathode - HeatLoop.Qairin1; 
HeatLoop.Qairin2 = zeros(10,10); 
for i = 1:10
    for j = 1:10
...
P_out = options.Pamb; 
H_E3 = property(E2,'h','kJ') - HeatLoop.Qinfuel2 - Heatloop.Qairin2;
