...
%Sizes for max cruise efficiency, 'Design,' and then examines
%takeoff and climb performance in 'OffDesign.'  Limiting SOFC power is
%determined by pushing the stack sized for max cruise efficiency to a
%limiting voltage threshold, and then using the gas turbine to add required
%power beyond that point
%'craft' function outputs Velocity, ambient conditions, thrust, power required and
...
Preq = (1/0.986)*[Demand.Pr_to,Demand.PrCb1,Demand.PrCb2,Demand.PrCb3,Demand.PrCb4,Demand.PrCb5,Demand.PrCb6,Demand.PrCb7,Demand.PrCb8,Demand.PrCruise];
...
A1.T = options.T0(1,1)*(1 + 0.5*0.4*0.82^2);%*ones(n,n); %Ambient Temperature, Kelvin
...
[param,Design,HL,intake] = run_cycle_3(options,FCArray,E1,A1);
mission = [];
%Size Turbine for max takeoff power
...
vflowairmax = Design.air_in*28.84./Cruise.dcruise; %Volumeflow at cruise, m^3/s
...
P = Preq(1,y); 
[paramod,OffDesign,HL,intake,Weight,A4] = run_cycle_3_od_exp(options,FCArray,E1,A1);
[OffDesign,Performance,Comp,Turb] = condition_3_exp(P,paramod,FCArray,y,OffDesign,Weight,A4);
