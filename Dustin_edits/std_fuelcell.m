function [FC,cath_out,anode_out] = std_fuelcell(options,cath_in)
...
error = 1;
V = .85;
frac_an_cool = .25+.3*(1-options.spu);
while any(any(abs(error)>1e-3))
    FC.i_total = (E0 + Ru*T./(2*F).*log(.85*.15/.15.*(P/100).^0.5) - V)./options.asr*1e4.*options.SOFC_area; %current estimate using average OCV and 0.85V
    FC.Qgen =  FC.hrxnmol.*FC.i_total./(2000.*F) - V.*FC.i_total./1000; %Heat generated by fuel cell, kW 
    error = (FC.Qgen.*(1-frac_an_cool) - Q_cath)./max(FC.Qgen.*(1-frac_an_cool),Q_cath);
    error(isnan(error)) = 0;
    V2 = V+.001;
    i_total2 = (E0 + Ru*T./(2*F).*log(.85*.15/.15.*(P/100).^0.5) - V2)./options.asr*1e4.*options.SOFC_area; %current estimate using average OCV and 0.85V
    Qgen2 =  FC.hrxnmol.*i_total2./(2000.*F) - V2.*i_total2./1000; %Heat generated by fuel cell, kW 
    error2 = (Qgen2.*(1-frac_an_cool) - Q_cath)./max(Qgen2.*(1-frac_an_cool),Q_cath);
    error2(isnan(error2)) = 0;
    dV_dQerror = .001./(error-error2);
    dV_dQerror(isinf(dV_dQerror)) = 0;
    V = min(1.4,max(.2,V + .7*error.*dV_dQerror));
%     V = V + .2*error.*max(0,(1.15-V));
end
