    ...
    for j = 1:1:n
        ...
        for k = 1:1:nn
            ...
            if P_req>max(P_shaft(:,k))
                [P,I] = max(P_shaft(:,k));
                battery_kJ(i,j) = battery_kJ(i,j) + (P_req - P)*mission.duration(k)*3600;
                weight.fuel(i,j) = weight.fuel(i,j) + FC.H2_used(I,k)*2*mission.duration(k)*3600;
            else
                weight.fuel(i,j) = weight.fuel(i,j) + interp1(P_shaft(:,k),FC.H2_used(:,k),P_req)*2*mission.duration(k)*3600;
            end
        end
    end
end
