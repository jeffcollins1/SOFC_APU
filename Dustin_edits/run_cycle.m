...
if parallel
    ...
    for i = 1:1:m*n
        [fuel(i),battery(i),P_sys_mission(i,:),eff_mission(i,:),FCV_mission(i,:),FCiden_mission(i,:)] = flight_profile(options,mission,vol_flow,i,n);
    end
end
for i = 1:1:m
    for j = 1:1:n
...
for k = 1:1:nn
    ...
    if P_req>max(P_shaft(:,k))
        ...
        %TSFC_mission(k) = mission.thrust(k)./fuel; 
