%Altitude bands, feet
function [Demand,Ppropmax,P_to,TrTO,time_to,velocityclimb,time_climb,P,T,dclimb,M,PrC,altitude,dcruise,PCruise,Tcruise,Vcruise,t1,t2,t3,t4,t5,t6,t7,t8,Fnet_to,TrCruise,clcruise,cdcruise] = craft(options)

PmaxHP = 4*60600*0.97*1.2; %Total horsepower required to generate max sea level thrust with electric propulsor 
Ppropmax = PmaxHP*0.7475; %Total propulsive power delivered by plant, kW
P_to = 0.5*Ppropmax; %Actual power used during takeoff
n=100; 
%Altitude bands, feet
...
m = options.TO_weight(1,1); %Total max takeoff weight, kg
...
Pr_to = 0.5*Ppropmax/0.986 + 1; %Actual power used to generate takeoff power based on motor efficiency
xx = size(time_to); 
...
ratio = cl./cd; 
Valt = velocityclimb.*sqrt(1.22./dclimb); 
%D = q.*S.*cd; 
...
TrC = W.*sin(aoaclimb) + Tr + m*accAvg; %Thrust required at max ROC
PrC = Pr.*sqrt(dclimb./1.22) + (W.*sin(aoaclimb)+ m*accAvg).*velocityclimb/1000 + 1000;
PrCa = TrC.*velocityclimb.*sqrt(dclimb./1.22)/1000 + 1000; %Total power required + 1 MW for electronic power
time_climb = 0.3048*44200/7; 
tclimbVec = linspace(time_to,time_climb,453); 
%plot(velocity,PrC)
%Power demand per altitude band
Demand.Pr_to = Pr_to; 
Demand.PrCb1 = (PrC(1,1) + PrC(1,5))*0.5;
Demand.PrCb2 = (PrC(1,6) + PrC(1,15))*0.5;
Demand.PrCb3 = (PrC(1,16) + PrC(1,45))*0.5;
Demand.PrCb4 = (PrC(1,46) + PrC(1,95))*0.5;
Demand.PrCb5 = (PrC(1,96) + PrC(1,195))*0.5;
Demand.PrCb6 = (PrC(1,196) + PrC(1,295))*0.5;
Demand.PrCb7 = (PrC(1,296) + PrC(1,395))*0.5;
Demand.PrCb8 = (PrC(1,396) + PrC(1,450))*0.5
% Cruise Performance
dcruise = min(dclimb); 
PCruise = min(P); 
Tcruise = min(T); 
Vcruise = 0.82*max(ss8); 
clcruise = (W-1100)/(0.5*dcruise*S*Vcruise^2);
cdcruise = 0.004 + (clcruise^2)/(pi*e*AR); 
ratiocruise = clcruise/cdcruise; 
TrCruise = (W-1100)/ratiocruise; 
Demand.PrCruise = sqrt(2*((W - 1100)^3)*(cdcruise^2)/(dcruise*S*clcruise^3))/(0.986*1000) + 1000;% TrCruise*Vcruise*sqrt(dcruise/1.22)/1000; %Power required at cruise, kW + 1 MW for power generation
Demand.PrCruise2 = Demand.PrCruise + 1000; 
%Gliding descent
...
time2= linspace(time_to,time_climb,mtot);
Pr_toVec = Pr_to.*ones(1,s1); 
[x,s2] = size(time2);
[x,s3] = size(tdescent); 
tx = time_to + time_climb; 
time3 = time_to + time_climb ; 
[timetotal] = [time1,time2,time3]; 
[Ptotal] = [Pr_toVec,PrCa,Demand.PrCruise];
velocitytotal = [V_toVec,velocityclimb,Vcruise];
% yyaxis left
% plot(timetotal,Ptotal)
% yyaxis right
% plot(timetotal,velocitytotal)
