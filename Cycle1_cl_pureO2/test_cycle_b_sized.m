...
%This version Tests Range for a previously designed cycle
...
for b = 1:10 %b index scales sofc stack
    
for z = 1:10 %z index selects ambient conditions and flight velocity at a particular altitude band
m1 = 10;
m2 = 10; 
OD.SOFC_area = CycleE(14,1)*(0.65 + b/20)*ones(10,10); %Fuel cell active area based index b
...
OD.T_otm = 1033*ones(m1,m2); %Operating temperature for OTM
...
Final.Mass(b,1) = (Final.TurbineMass(b,1) + Final.CompMass(b,1) + Final.HXMass(b,1) + Final.OTMMass(b,1) + Final.SOFCMass(b,1)); %Find total mass required for power plant index b to perform across all power demands and ambient conditions with 10% for hardware
...
Final.Table = [Final.Payload,Final.TurbineMass,Final.CompMass,Final.HXMass,Final.OTMMass, Final.SOFCMass, Final.Mass,Final.TSFC_to,Final.TSFC_cruise,Final.H2storedtotal,Final.H2reserve]; 
% m1 = 10;
% m2 = 10; 
