...
options.Tamb= 298*ones(n1,n2); %Ambient Temperature at sea level Kelvin;
options.Pamb= 100*ones(n1,n2); %Ambient Pressure at sea level, kPa
n = 10;
T = 1023;%*ones(n,n);

options.motor_eff = 0.986*ones(10,10);
options.motor_Qgen = 1000; 
ASR = 0.15;%*ones(n,n);
...
options.Pratiomax = options.P_fc./Cruise.PCruise;
options.Pratiomin = options.P_fc./options.P0; 
A1.O2 = 0.21;%*ones(n,n); %Molar flow oxygen
A1.N2 = 0.79;%*ones(n,n); %Molar flow nitrogen
A1.T = 300;%*ones(n,n); %Ambient Temperature, Kelvin
A1.P = 100;%*ones(n,n); %Ambient Pressure, kPa
L = 9;
W = 9;
P = 10;
util = 0.2;%(ones(n,1)*linspace(0.1,0.5,n))';%*ones(n,n);
r = 0;%*ones(n,n); 
...
for i = 1:1:n
    for j = 1:1:10
        ...
        FCArray.Pamb = options.Pamb; 
        FCArray.P0 =  options.Pamb; 
        FCArray.airin = 1*ones(10,10);
    end
end
...
[param,Design,HL,intake] = run_cycle_2(options,FCArray,E1,A1);


%Cruise Performance
Cruise.air_in_min = Design.air_in*Cruise.dcruise/1.22; %minimum Molar flow of air at the same flow rate as ground condition
Cruise.air_in_max = 2*Cruise.air_in_min; %Maximum airflow at cruise assuming compressor runs at 50% power for ground condition
Cruise.Cells = Design.Cells;
Cruise.PrCruise = Demand.PrCruise;
Cruise.Mach = M(1,450); 
Profile = zeros(10,15);
...
for i = 1:1:n
    for j = 1:1:10
        O2util =  i*0.045;
        air_in = (0.5 + 0.05*j + 0.1*y)*Design.air_in*(OffDesign.altdata(y,5)/1.22); 
        A1.O2 = 0.21*air_in;
        A1.N2 = 0.79*air_in;
        ...
        [FC,E1] = FuelCell_H2(T,ASR,A1,L,W,n,Cruise.Cells,P,O2util,util,r,options);
        FCArray.Power(i,j) = FC.Power;
        ...
        FCArray.Tamb(i,j) = OffDesign.altdata(y,4);
        FCArray.Pamb(i,j) = OffDesign.altdata(y,3);
    end
 
end
FCArray.P0 = OffDesign.altdata(y,3)*ones(10,10); 
P = Preq(1,y); 
[Weight,paramod] = run_cycle_2Cruise(options,FCArray,E1,A1,Cruise);
[OffDesign,Performance,Comp,Turb] = condition_2(P,Cruise,paramod,FCArray,y,OffDesign,Weight);
OffDesign.Performance(y,1:15) = Performance(y,1:15);
OffDesign.CompMass(1,y) = Comp; 
OffDesign.TurbMass(1,y) = Turb;
end
%OffDesign.Performance(1,1:15) = Design.Performance(1,1:15); 
TSFCcruise = OffDesign.H2usedactual*1000000/Cruise.TrCruise; %g/kNs
...
Final.TotalMass = OffDesign.TrueMass;
