function [OffDesign] = condition_2b(Demand,param,FCArray)
P_req = Demand.PrCruise;
P_gen = param.NetPower;
error = abs(P_req - P_gen);
min_err = min(min(error));
Qbal = param.Qbalance; 
H2_used = param.H2_used;
H2_heat=zeros(10,10);
hrxnmol = param.hrxnmol;
for a = 1:10
    for b = 1:10
        if Qbal(a,b) < 0
...
OffDesign.Qoutcondenser = param.Qremovefuel;
OffDesign.Qairpreheat = param.preheat_air; 
OffDesign.Comp1Work = param.C1_work;
OffDesign.Turb1Work = param.T1_work;
OffDesign.BlowerWork = param.BlowerWork; 
OffDesign.FCPower = FCArray.Power;
selection = zeros(18,15);
for c = 1:9
    for d = 1:10
        if P_req >= P_gen(c,d) && P_req <= P_gen((c+1),d)
            fraction = -(P_req - param.NetPower((c+1),d))/(-(param.NetPower(c,d) - param.NetPower((c+1),d)));
