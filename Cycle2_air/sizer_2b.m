function [Design] = sizer_2b(options,param,Weight,HL)
Qbal = HL.Qexcess; 
H2_heat = zeros(10,10);
...
for i = 1:10
    for j =1:10
if Qbal(i,j) < 0
    H2_heat(i,j) = -Qbal(i,j)/param.hrxnmol(1,1); 
    Qbal(i,j) = 0; 
end
    end
end
H2used = param.H2_used + H2_heat; %Total H2 used in kmol/s
FTE = param.NetPower./(param.hrxnmol.*H2used);
point = max(max(FTE));
[x,y] = find(FTE==point); 
batteryboost = 0; 
P_net = param.NetPower(x,y); 
scale = (1-batteryboost)*options.PrCruise/P_net; 
...
Design.H2usedactual = Design.H2_usednom + scale*H2_heat(x,y)*2; %Actual fuel burn at max cruise efficiency, kg/s
...
Design.P_den = P_net/Weight.Total(x,y); 
