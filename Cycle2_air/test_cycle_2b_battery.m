...
BatteryMass = Battery.Total/1260; %Required battery mass to assist with takeoff and climb
...
Preq = [Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise,Demand.PrCruise2];
OffDesign.altdata(1,:) = [climb.altitude(1,1),climb.velocityclimb(1,1),climb.P(1,1),climb.T(1,1),climb.d(1,1),Demand.PrCruise,M(1,1)];
OffDesign.altdata(2,:) = [climb.altitude(1,5),climb.velocityclimb(1,5),climb.P(1,5),climb.T(1,5),climb.d(1,5),Demand.PrCruise,M(1,5)];
OffDesign.altdata(3,:) = [climb.altitude(1,15),climb.velocityclimb(1,15),climb.P(1,15),climb.T(1,15),climb.d(1,15),Demand.PrCruise,M(1,15)];
OffDesign.altdata(4,:) = [climb.altitude(1,45),climb.velocityclimb(1,45),climb.P(1,45),climb.T(1,45),climb.d(1,45),Demand.PrCruise,M(1,45)];
OffDesign.altdata(5,:) = [climb.altitude(1,95),climb.velocityclimb(1,95),climb.P(1,95),climb.T(1,95),climb.d(1,95),Demand.PrCruise,M(1,95)];
OffDesign.altdata(6,:) = [climb.altitude(1,195),climb.velocityclimb(1,195),climb.P(1,15),climb.T(1,195),climb.d(1,195),Demand.PrCruise,M(1,195)];
OffDesign.altdata(7,:) = [climb.altitude(1,295),climb.velocityclimb(1,295),climb.P(1,295),climb.T(1,295),climb.d(1,295),Demand.PrCruise,M(1,295)];
OffDesign.altdata(8,:) = [climb.altitude(1,395),climb.velocityclimb(1,395),climb.P(1,395),climb.T(1,395),climb.d(1,395),Demand.PrCruise,M(1,395)];
OffDesign.altdata(9,:) = [climb.altitude(1,450),climb.velocityclimb(1,450),climb.P(1,450),climb.T(1,450),climb.d(1,450),Demand.PrCruise,M(1,450)];
OffDesign.altdata(10,:) =[44000,Cruise.Vcruise,Cruise.PCruise,Cruise.TCruise,Cruise.dcruise,Demand.PrCruise,0.82];
OffDesign.altdata(11,:) =[44000,Cruise.Vcruise,Cruise.PCruise,Cruise.TCruise,Cruise.dcruise,Demand.PrCruise2,0.82];
...
OffDesign.Performance = zeros(11,15); 
OffDesign.CompMass = zeros(1,11);
OffDesign.TurbMass = zeros(1,11);
OffDesign.HXmass = zeros(1,11); 
Final.HTSM_mass = zeros(1,11);  
Final.TotalMass = zeros(1,11);
Final.Compmass = zeros(1,11);
Final.Turbmass=zeros(1,11);
Final.SOFCmass = zeros(1,11);
Final.HXmass=zeros(1,11); 
Final.Pden = zeros(1,11); 
Final.Penalty = zeros(1,11); 
Final.Payload = zeros(1,11);
Final.TSFCto= zeros(1,11);
Final.TSFCcr = zeros(1,11);
Final.SOFCmass = zeros(1,11); 
Final.Battery = zeros(1,11); 
Final.TotalMass = zeros(1,11);
PerformanceTable = zeros(121,15);
for q = 1:11
for y = 1:11
for i = 1:1:n
    for j = 1:1:10
...
OffDesign.SOFCmass(1,y) = Weight.sofc(1,1); 
end
l = q*11;
m = l-10; 
...
Final.Battery(1,q) = BatteryMass;
Final.TotalMass(1,q) = (OffDesign.compmass + OffDesign.turbmass + OffDesign.SOFCmass(1,q) + Final.HXmass(1,q) + Final.HTSM_mass(1,q) + Final.Battery(1,q))*1.1; %Total mass of all components based on maximum requirements and adding 10% for hardware
...
TSFCcruise1 = OffDesign.Performance(10,4)*1000000/Cruise.TrCruise; %g/kNs
TSFCcruise2 = OffDesign.Performance(11,4)*1000000/Cruise.TrCruise;  %g/kNs
TSFCcruise = ((Battery.Total/1000)*TSFCcruise2 + (OffDesign.Cruise_time - Battery.Total/1000)*TSFCcruise2)/OffDesign.Cruise_time; 
...
Final.Table = [Final.Payload', Final.Turbmass',Final.Compmass',Final.HXmass',Final.SOFCmass', Final.HTSM_mass',Final.Battery',Final.Penalty',Final.TotalMass',Final.TSFCto',Final.TSFCcr',Final.TotalFuel'];